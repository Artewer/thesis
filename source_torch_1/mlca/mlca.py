...
from source_torch.pysats import PySats
from source_torch.mlca.mlca_economies import MLCA_Economies
from source_torch.util import key_to_int, timediff_d_h_m_s
...
def mlca_mechanism(SATS_domain_name=None, SATS_auction_instance_seed=None, Qinit=None, Qmax=None, Qround=None, NN_parameters=None, MIP_parameters=None, scaler=None,
    ...
    # init_bids, init_fitted_scaler = init_bids_and_fitted_scaler
    if init_bids is not None:
        E.set_initial_bids(initial_bids=init_bids, fitted_scaler=init_fitted_scaler, active_learning_algorithm= active_learning_algorithm) # (*) use self defined inital bids | Line 1
    else:
        E.set_initial_bids(active_learning_algorithm= active_learning_algorithm)  # (*) create inital bids B^0, uniformly sampling at random from bundle space | Line 2
    ...
    while CHECK:
        ...
        for bidder in E.bidder_names:
            ...
            for marginal_economy in sampled_marginal_economies:
                ...
                print(marginal_economy)
                for bidder in E.economies_names[marginal_economy]:
        ...
        Rmax = max(E.get_number_of_elicited_bids().values())
        CHECK = Rmax <= (E.Qmax-E.Qround)
        #if E.mlca_iteration <= 1: CHECK = True
        print('Rmax: ', Rmax)
        print('Qmax - Qround: ', E.Qmax)
        print('CHECK: ', CHECK)
