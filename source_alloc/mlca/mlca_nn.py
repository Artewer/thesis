...
"""
FILE DESCRIPTION:
This file implements the class BidderNN (Bidder Valuation Neural Network) and MLCA_ALLOC.
The class BidderNN is a subclass the nn.Module, the basis of all neural network modules, receives the model parameters and sets the appropriate values for each layer of the Deep Neural Network. It serves as a constructor of a network that is invoked by the class MLCA_ALLOC.
The class MLCA_ALLOC implements the Deep Neural Network responsible for producing an optimal valuation estimation.
The class BidderNN has following functionalities:
    0.CONSTRUCTOR: __init__(self, M, model_parameters)
        model_parameters = Parameters specifying allocation model, includes model architecture (as array of integers representing layers) dropout , and dropout_probability
        The weights are initialized using the XAVIER uniform initializer. The sequential NN consists of dense layers with ReLU.  
    1.METHOD: forward(self, x)
        This method defines the computation performed at every call. 

The class MLCA_ALLOC has the following funtionalities:
...
class MLCA_NN:
    ...
    def __get_reg_loss(self, model):
        ...
        l1_regularization, l2_regularization = torch.FloatTensor([0]).to(self.device), torch.FloatTensor([0]).to(
            self.device)
