...
# from source_alloc.mlca.mlca_nn_alloc_max import MLCA_NN_ALLOC_MAX
...
class MLCA_Economies:
    ...
    def get_info(self, final_summary=False):
        ...
        if not final_summary: logging.warning('Efficiency, welfare and current baseline given elicited bids from iteration 0-%s: %s, %s, %s\n', self.mlca_iteration-1,self.efficiency_per_iteration[self.mlca_iteration-1], self.welfare_per_iteration[self.mlca_iteration-1], self.baseline)
        # welfare_per_iteration index changed to mlca_iteration-1 in case previous SATS instances are used as baseline
    ...
    def calculate_efficiency_welfare_per_iteration(self):
        ...
        #self.exp_avg_rule[self.mlca_iteration+1] = self.gamma * self.welfare_per_iteration[self.mlca_iteration]  + (1 - self.gamma) * self.exp_avg_rule[self.mlca_iteration]
    ...
    def optimization_step(self, economy_key, bidder_specific_constraints=None):
        ...
        #tmp = X.fit_alloc(self.exp_avg_rule[self.mlca_iteration], model_parameters_alloc=self.NN_Alloc_parameters).detach().to("cpu").numpy()
        tmp = X.fit_alloc(self.baseline, model_parameters_alloc=self.NN_Alloc_parameters).detach().to("cpu").numpy()
