...
def pvm(scaler=None, caps=None, L=None, parameters=None, epochs=None, batch_size=None, model_name=None, sample_weight_on=False, sample_weight_scaling=False, min_iteration=None, seed_instance=None, regularization_type=None,
        Mip_bounds_tightening=None, warm_start=None, configdict=None):

    start0 = time.time()
    if configdict is not None:
        model_name = configdict['SATS_domain_name']
        seed_instance = configdict['SATS_auction_instance_seed']
        c0 = configdict['caps'][0]
        ce = configdict['caps'][1]
        L = configdict['L']
        parameters = configdict['NN_parameters']
        ...
        regularization_type = configdict['regularization_type']
        min_iteration = 1
        Mip_bounds_tightening = configdict['MIP_parameters']['mip_bounds_tightening']
    ...
    if starter == 'mlca_extra':
        E.set_initial_bids_pvm_extra(
            seeds_random_bids=seeds_random_bids)  # create inital bids V^0, uniformly sampling at random from bundle space

    if starter == 'unif_empty':
        E.set_initial_bids_pvm_unif_empty(
            seeds_random_bids=seeds_random_bids)  # create inital bids V^0, uniformly sampling at random from bundle space
    ...
    RESULT = OrderedDict([('seed',seed_instance),('eff',eff),('value_SATS',value_SATS), ('alloc',alloc), ('payments',E.payments), ('T',T), ('iteration',E.iteration), ('NN losses',E.losses) ,('total bounds',E.total_bounds), ('elapsed time',E.elapsed_time_mip), ('actual queries',E.actual_queries_elicitation),('elicited_bids',E.elicited_bids)])
    return(RESULT)
