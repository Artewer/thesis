...
import source_torch.util as util_torch
...
class MLCA_Economies:
    ...
    def set_initial_bids(self, active_learning_algorithm, initial_bids=None, fitted_scaler=None):
        logging.info('INITIALIZE BIDS')
        logging.info('-----------------------------------------------\n')
        if initial_bids is None: # Uniform sampling Do here
            if active_learning_algorithm == 'unif':
                _elicited, _fitted = util_torch.initial_bids_mlca_unif(SATS_auction_instance = self.SATS_auction_instance,
                                                                                 number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                 scaler=self.scaler)
            elif active_learning_algorithm == 'active_learning':
                _elicited, _fitted = util_torch.initial_bids_mlca_active_learning(SATS_auction_instance=self.SATS_auction_instance,
                                                                        number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                        scaler=self.scaler)
                
                
            elif active_learning_algorithm == 'fft':
                _elicited, _fitted = util_torch.initial_bids_mlca_fft(SATS_auction_instance=self.SATS_auction_instance,
                                                                        number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                        scaler=self.scaler)

            elif active_learning_algorithm == 'fft_reverse':
                _elicited, _fitted = util_torch.initial_bids_mlca_fft_reverse(SATS_auction_instance = self.SATS_auction_instance,
                                                                                    number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                    scaler=self.scaler)
            elif active_learning_algorithm == 'gali':
                _elicited, _fitted = util_torch.initial_bids_mlca_gali(SATS_auction_instance = self.SATS_auction_instance,
                                                                                    number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                    scaler=self.scaler)
    ...
    def set_initial_bids_mlca_extra(self, initial_bids=None, fitted_scaler=None):
        ...
        if initial_bids is None:  # Uniform sampling
            ...
            if not (list(initial_bids.keys()) == self.bidder_names):
                ...
                self.elicited_bids = deepcopy(_elicited)
                # added initial bids to capture initialization
                self.initial_update_elicited_bidsbundles = deepcopy(_elicited)
                self.fitted_scaler = fitted_scaler  # fitted scaler to initial bids
                self.Qinit = [v[0].shape[0] for k, v in initial_bids.items()]
