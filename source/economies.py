...
class Economies:
    ...
    def set_initial_bids_pvm_unif_empty(self, seeds_random_bids):
        for key, value in self.economies.items():
            logging.debug(key)
            self.elicited_bids[key] = util.initial_bids_pvm_unif_empty(value_model=self.value_model, c0=self.c0, bidder_ids=value, scaler=self.scaler)  # TRUE UNIFORM SAMPLING
    ...
    def set_initial_bids_pvm_extra(self, seeds_random_bids):
        for key, value in self.economies.items():
            logging.debug(key)
            self.elicited_bids[key] = util.initial_bids_pvm_extra(value_model=self.value_model, c0=self.c0, bidder_ids=value, scaler=self.scaler)  # TRUE UNIFORM SAMPLING
            # self.elicited_bids[key] = util.initial_bids_pvm(value_model=self.value_model, c0=self.c0, bidder_ids=value, scaler=self.scaler, seed=seeds_random_bids)  # SATS RANDOM SAMPLING VIA NORMAL DISTRIBUTION
            self.fitted_scaler[key] = self.elicited_bids[key][1]
