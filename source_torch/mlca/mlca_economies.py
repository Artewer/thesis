...
class MLCA_Economies:
    ...
    def set_initial_bids(self, active_learning_algorithm, initial_bids=None, fitted_scaler=None, presampled_n = 10, presampled_algorithm = 'unif'):
        logging.info('INITIALIZE BIDS')
        logging.info('-----------------------------------------------\n')
        if initial_bids is None: # Uniform sampling Do here
            if active_learning_algorithm == 'unif':
                _elicited, _fitted = util.initial_bids_mlca_unif(SATS_auction_instance = self.SATS_auction_instance,
                                                                                 number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                 scaler=self.scaler)
            elif active_learning_algorithm == 'active_learning':
                _elicited, _fitted = util.initial_bids_mlca_active_learning(SATS_auction_instance=self.SATS_auction_instance,
                                                                        number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                        scaler=self.scaler)
                
                
            elif active_learning_algorithm == 'fft':
                _elicited, _fitted = util.initial_bids_mlca_fft(SATS_auction_instance=self.SATS_auction_instance,
                                                                        number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                        scaler=self.scaler)

            elif active_learning_algorithm == 'fft_reverse':
                _elicited, _fitted = util.initial_bids_mlca_fft_reverse(SATS_auction_instance = self.SATS_auction_instance,
                                                                                    number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                    scaler=self.scaler)
            elif active_learning_algorithm == 'gali':
                _elicited, _fitted = util.initial_bids_mlca_gali(SATS_auction_instance = self.SATS_auction_instance,
                                                                                    number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                    scaler=self.scaler)
            elif active_learning_algorithm == 'galo':
                _elicited, _fitted = util.initial_bids_mlca_galo(presampled_n=presampled_n, presampled_algorithm = presampled_algorithm, SATS_auction_instance = self.SATS_auction_instance,number_initial_bids=self.Qinit, bidder_names=self.bidder_names,
                                                                                    scaler=self.scaler)
    ...
    def next_queries(self, economy_key, active_bidder):
        if not self.economy_status[economy_key]:  # check if economy already has been calculated prior
            ...
            print('First time in optimization step.')
        ...
        if self.check_bundle_contained(bundle=self.argmax_allocation[economy_key][active_bidder][0], bidder=active_bidder): # Check if argmax bundle already has been queried in R or S
            ...
            print(f'*** Query existed, resolving- active bidder is bidder {active_bidder} ***')
